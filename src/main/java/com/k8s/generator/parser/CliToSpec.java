 ...
 * <ul>
 *   <li><b>Cluster name</b>: clu-{num}-{type}-{engine} (e.g., "clu-m1-pt-kubeadm")</li>
 ...
 * // spec.primaryCluster().name() = "clu-m1-pt-kubeadm"
 * // spec.primaryCluster().type() = ClusterType.Kubeadm
 * // spec.primaryCluster().nodes() = NodeTopology.of(1, 0)
...
public final class CliToSpec implements SpecConverter {
    ...
    private ClusterSpec getClusterSpec(final GenerateCommand cmd,
        ...
        Spec spec = switch (clusterType) {
            case Kubeadm ku -> {
                ...
                yield Spec.builder()
                        .nodes(NodeTopology.of(nw[0], nw[1]))
    ...
    @Builder
    private record Spec(NodeTopology nodes, CniType cni) {
        static Spec empty() {
            return new Spec(NodeTopology.of(0, 0), null);
        }
    }
    ...
    private ClusterType parseClusterType(String clusterType) {
        ...
        return switch (normalized) {
            ...
            default -> throw new IllegalArgumentException(
                    String.format(
                            "Unsupported cluster type: '%s'. Supported: kubeadm, mgmt",
                            clusterType
                    )
            );
        };
